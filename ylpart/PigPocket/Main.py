        ...
        for i in userList:
                if form.userId.data==i.get_nric():
                    if form.userPass.data==i.get_password():
                        ...
                        tkinter.messagebox.showerror("Alert","Wrong password or username")
                        return redirect("/")
...
def fundtransfer():
    ...
    de=MainProcess.deposits(session['userid'],"Jan 2018")
    we=MainProcess.spendings(session["userid"],"Jan 2018")
    dbsd=MainProcess.totalforbankdeposit(session['userid'],"DBS")
    ocbcd=MainProcess.totalforbankdeposit(session['userid'],"OCBC")
    uobd=MainProcess.totalforbankdeposit(session['userid'],"UOB")
    dbsw = MainProcess.totalforbankwithdraw(session['userid'], "DBS")
    ocbcw = MainProcess.totalforbankwithdraw(session['userid'], "OCBC")
    uobw= MainProcess.totalforbankwithdraw(session['userid'], "UOB")

    return render_template('fundtransfer.html',dbsd=dbsd,ocbcd=ocbcd,uobd=uobd,dbsw=dbsw,ocbcw=ocbcw,uobw=uobw,de=de,we=we,transaction=transaction,totalw=totalw,totald=totald,Transaction=userlist,form=form,totalmonthdeposit=totalmonthdeposit,totalmonthwithdraw=totalmonthwithdraw,totalprevwithdraw=totalprevwithdraw,totalprevdeposit=totalprevdeposit,month=month,detailsList=detailsList,prevmonth=prevmonth)
...
def rewards():
    ...
    enddate = str(2018) + "-" + str(2) + "-" + str(28)
    enddate2= str(2018) + "-" + str(3) + "-" + str(31)
    print(nowdate)
    print(enddate)
    pts = processpoint.calculate_point(session['userid'])
    if str(nowdate).startswith(enddate):
        newpt = open("file/point.txt", "a")
        newpt.write(session['userid'] + "," + str(pts) + "\n")
    elif str(nowdate).startswith(enddate2):
        newpt = open("file/point.txt", "a")
        newpt.write(session['userid'] + "," + str(pts) + "\n")
